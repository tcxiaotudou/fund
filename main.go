...
func main() {
	...
	for name, code := range constant.EtfGroups {
		...
		// rsi小于35 && rsi离最低点小于5天 或者 rsi小于45 && 最低点大于35 && 最高点大于70 && rsi离最低点小于5天
		if (etfRsiData.Now < 35 && etfRsiData.NowToLowDays <= 5) ||
			(etfRsiData.High >= 70 && etfRsiData.Now <= 45 && etfRsiData.Low >= 35 && etfRsiData.NowToLowDays <= 5) {
